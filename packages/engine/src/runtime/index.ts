...
import { type EngineRuntimeContext } from "#api";
...
export type RuntimeEngineLayer = Layer.Layer<
 ...
 *
 *   Tier 1 — Foundation
 *     EngineStore, RuntimeLocalWorkspace, LiveExecutionMgr
 *
 ...
 *     ExecutionResolver
 */
export const createRuntimeEngineLayer = (
  input: RuntimeEngineOptions & {
  ...
  return Layer.mergeAll(
    ...
    tier4_source,
    tier5_execution,
  ) as RuntimeEngineLayer;
};

export type EngineRuntime = {
  persistence: LocalEnginePersistence;
  localInstallation: LocalInstallation;
  managedRuntime: ManagedRuntime.ManagedRuntime<EngineRuntimeContext, never>;
  runtimeLayer: RuntimeEngineLayer;
  close: () => Promise<void>;
};

export const provideEngineRuntime = <A, E, R>(
  effect: Effect.Effect<A, E, R>,
  runtime: EngineRuntime,
): Effect.Effect<A, E | never, Exclude<R, EngineRuntimeContext>> =>
  effect.pipe(Effect.provide(runtime.managedRuntime));

export const createEngineRuntime = (
  options: RuntimeEngineOptions,
): Effect.Effect<EngineRuntime, Error> =>
  Effect.gen(function* () {
    ...
    const persistence = yield* createLocalEnginePersistence(
    ...
    const concreteRuntimeLayer = createRuntimeEngineLayer({
      ...options,
      store: rows,
      localWorkspaceState: runtimeLocalWorkspaceState,
    ...
    return {
      ...
      localInstallation,
      managedRuntime,
      runtimeLayer: concreteRuntimeLayer as RuntimeEngineLayer,
      close: async () => {
