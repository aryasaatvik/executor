...
import { withEngineClient } from "./test-http-client";
...
describe("execution-http", () => {
  it.scoped("creates and persists an execution through the HTTP API", () =>
    Effect.gen(function* () {
      ...
      const createExecution = yield* withEngineClient(
        {
          runtime,
      ...
      const getExecution = yield* withEngineClient(
        {
          runtime,
          accountId: installation.accountId,
        },
  ...
  it.scoped("persists executionSessionId and closes execution sessions through the HTTP API", () =>
    Effect.gen(function* () {
      ...
      const closeResult = yield* withEngineClient(
        {
          runtime,
          accountId: installation.accountId,
        },
