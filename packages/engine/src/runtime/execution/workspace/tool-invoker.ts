...
export const createWorkspaceToolInvoker = (input: {
  ...
  const localStorageLayer = makeLocalStorageLayer({
    installationStore: LocalInstallationStore,
    workspaceConfigStore: input.workspaceConfigStore,
  });
  const workspaceDatabaseLayer =
    input.runtimeLocalWorkspace === null
      ? null
      : Layer.succeed(
          WorkspaceDatabase,
          makeWorkspaceDatabase(input.runtimeLocalWorkspace),
        );
  const provideWorkspaceServices = <A, E, R>(effect: Effect.Effect<A, E, R>) =>
    effect.pipe(
      Effect.provide(
        workspaceDatabaseLayer === null
          ? Layer.mergeAll(localStorageLayer, workspaceConfigLayer)
          : Layer.mergeAll(
              localStorageLayer,
  ...
  const invokePersistedTool = (invocation: {
    ...
    provideRuntimeLocalWorkspace(
      provideWorkspaceServices(Effect.gen(function* () {
