...
import { withEngineClient } from "./execution/test-http-client";
...
const upsertLocalSecret = (runtime: EngineRuntime, input: {
  id: string;
  name: string;
...
const createPersistedGoogleSource = (input: {
  ...
  Effect.gen(function* () {
    ...
    }).pipe((effect) => provideEngineRuntime(effect, input.runtime));
  });
  ...
  Effect.either(effect).pipe(
    Effect.flatMap((result) =>
      Either.isLeft(result)
        ...
        : Effect.fail(runtimeEffectError("engine-runtime.test", "Expected effect to fail")),
    ),
  );
...
describe("control-plane-runtime", () => {
  it.scoped("writes local source changes through executor.jsonc", () =>
    Effect.gen(function* () {
      ...
      const createdSource = yield* withEngineClient(
      ...
      const removed = yield* withEngineClient(
        { runtime, accountId: installation.accountId },
        (client) =>
  ...
  it.scoped("manages local secrets through provider-backed metadata rows", () =>
    Effect.gen(function* () {
      ...
      const created = yield* withEngineClient(
        { runtime },
        (client) =>
      ...
      const listed = yield* withEngineClient(
        { runtime },
      ...
      const updated = yield* withEngineClient(
        { runtime },
        (client) =>
  ...
  it.scoped("reports safe secret storage options for the current platform", () =>
    Effect.gen(function* () {
      ...
      const config = yield* withEngineClient(
        { runtime },
  ...
  it.scoped("captures credential requests through the local HTML flow without persisting raw tokens", () =>
    Effect.gen(function* () {
      ...
      yield* persistSource(runtime.persistence.rows, localSource, {
        actorAccountId: installation.accountId,
      }).pipe((effect) => provideEngineRuntime(effect, runtime), Effect.orDie);
      ...
      const interactionFiber = yield* Effect.gen(function* () {
        ...
        (effect) => provideEngineRuntime(effect, runtime),
        Effect.fork,
      );
      ...
      const page = yield* withEngineClient(
        { runtime },
        (client) =>
      ...
      const submittedPage = yield* withEngineClient(
        { runtime },
        (client) =>
...
`,
      ...
      const inspection = yield* withEngineClient(
        { runtime, accountId: installation.accountId },
        (client) =>
  ...
  it.scoped("reuses existing Google provider grants for single connects and clears orphan state", () =>
    Effect.gen(function* () {
      ...
      const oauthClient = yield* withEngineClient(
        { runtime },
        (client) =>
      ...
      const result = yield* withEngineClient(
        { runtime },
        (client) =>
  ...
  it.scoped("preserves the existing provider refresh token when callback expansion omits refresh_token", () =>
    Effect.gen(function* () {
      ...
      const callbackPage = yield* withEngineClient(
        { runtime },
        (client) =>
      ...
      const refreshedSource = yield* withEngineClient(
        { runtime },
        (client) =>
          client.sources.get({
            path: {
  ...
  it.scoped("revokes shared provider grants through the API and disconnects linked sources", () =>
    Effect.gen(function* () {
      ...
      const disconnectedDrive = yield* withEngineClient(
        { runtime },
        (client) =>
          client.sources.get({
            path: {
      ...
      const disconnectedGmail = yield* withEngineClient(
        { runtime },
        (client) =>
          client.sources.get({
            path: {
