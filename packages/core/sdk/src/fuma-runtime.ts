...
const makeSafeFumaQuery = <TSchema extends AnySchema>(
  ...
  const query: FumaQuery<TSchema> = {
    ...
    jsonCount: (name, value) => db.jsonCount(table(name), value),
    jsonGroupCount: (name, value) => db.jsonGroupCount(table(name), value),
    jsonTimeBuckets: (name, value) => db.jsonTimeBuckets(table(name), value),
    jsonStats: (name, value) => db.jsonStats(table(name), value),
    jsonPage: (name, value) => db.jsonPage(table(name), value),
    transaction: (run) =>
