...
import { SourceIdSchema } from "@executor/control-plane/model";
...
export const synchronizeLocalWorkspaceState = (input: {
  ...
  Effect.gen(function* () {
    ...
    const embeddingDimensions =
      typeof input.loadedConfig.config?.semanticSearch?.dimensions === "number"
        ? input.loadedConfig.config.semanticSearch.dimensions
        : undefined;
    ...
    yield* Effect.gen(function* () {
      ...
      for (const removedRow of removedRows) {
        yield* removeSourceEmbeddings(removedRow.sourceId).pipe(
          Effect.catchAll((error) =>
            Effect.logWarning(
              `Failed removing embeddings for deleted source ${removedRow.sourceId}: ${error instanceof Error ? error.message : String(error)}`,
            ).pipe(Effect.asVoid),
          ),
        );
