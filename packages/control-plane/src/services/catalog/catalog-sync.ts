...
export const RuntimeSourceCatalogSyncLive = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    return SourceCatalogSync.of({
      sync: (input) => provideCatalogSyncDependencies(syncSourceCatalogProgram(input)) as Effect.Effect<void, Error, never>,
      persistMcpCatalogSnapshotFromManifest: (input) =>
        provideCatalogSyncDependencies(persistMcpCatalogSnapshotProgram(input)) as Effect.Effect<void, Error, never>,
    });
  }),
);
