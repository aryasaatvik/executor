...
import { LocalSourceArtifactMissingError } from "../engine/local-errors";
...
const mapInspectionError = (
  ...
  cause: unknown,
  details: string,
): EngineNotFoundError | EngineStorageError => {
  if (cause instanceof EngineNotFoundError) {
    return cause;
  }

  if (cause instanceof EngineStorageError) {
    return cause;
  }

  if (cause instanceof LocalSourceArtifactMissingError) {
