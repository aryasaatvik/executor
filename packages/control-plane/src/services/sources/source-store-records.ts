...
import { refreshWorkspaceSourceTypeDeclarationsInBackground } from "../catalog/type-declarations";
import type { LoadedLocalExecutorConfig } from "../engine/local-config";
import {
...
import { getSourceAdapter } from "../engine/source-adapters";
import {
...
} from "./source-store-auth";
import {
  sourceAuthFromConfigInput,
  trimOrNull,
} from "./source-store-config";
import { catalog_tool, catalog_revision } from "../engine/db";
import { stableSourceCatalogId } from "./source-definitions";
import { loadSourceStatus, type SourceStatusRecord } from "../engine/db";
import {
  makeWorkspaceDatabase,
} from "../engine/workspace-database";
...
> =>
  Effect.gen(function* () {
    ...
    const baseSource = (yield* adapter.validateSource({
      ...
      binding: sourceConfig.binding as Record<string, import("../../model/index").JsonValue>,
