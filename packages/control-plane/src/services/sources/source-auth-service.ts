...
import { createPersistedMcpOAuthSourceAuth } from "../engine/mcp-auth-provider";
import {
...
import type { WorkspaceStorageServices } from "../engine/local-storage";
import { EngineStore, type EngineStoreShape } from "../engine/store";
import { runtimeEffectError } from "../engine/errors";
...
const reusePendingMcpSourceAuthSession = (input: {
  ...
  endpoint: string;
  preferFreshAuthorization?: boolean;
}): Effect.Effect<Extract<McpSourceConnectResult, { kind: "oauth_required" }> | null, Error> =>
  Effect.gen(function* () {
    const actorAccountId = input.actorAccountId ?? null;
    const pendingSessions = (yield* input.rows.sourceAuthSessions.listByWorkspaceId(input.source.workspaceId))
      .filter((session) =>
        session.sourceId === input.source.id
        && session.status === "pending"
        && session.credentialSlot === "runtime"
        && session.actorAccountId === actorAccountId
      )
    ...
    if (input.preferFreshAuthorization === true) {
      const now = Date.now();
      yield* Effect.forEach(
        pendingSessions,
        (session) =>
          input.rows.sourceAuthSessions.update(session.id, {
            status: "cancelled",
            errorText: "Superseded by a newer source auth request",
            completedAt: now,
            updatedAt: now,
          }),
    ...
    let reusableSession: SourceAuthSession | null = null;
    let authorizationUrl: string | null = null;

    for (const session of pendingSessions) {
      try {
        const sessionData = decodeMcpSourceAuthSessionData(session);
        const sessionAuthorizationUrl = trimOrNull(sessionData.authorizationUrl);
        if (
          normalizeEndpoint(sessionData.endpoint) === input.endpoint
          && sessionAuthorizationUrl !== null
        ) {
          reusableSession = session;
          authorizationUrl = sessionAuthorizationUrl;
          break;
        }
      } catch {
        continue;
    ...
    if (reusableSession === null || authorizationUrl === null) {
      return null;
    }

    const now = Date.now();
    yield* Effect.forEach(
      pendingSessions.filter((session) => session.id !== reusableSession.id),
      (session) =>
        input.rows.sourceAuthSessions.update(session.id, {
          status: "cancelled",
          errorText: "Superseded by a newer pending source auth session",
          completedAt: now,
          updatedAt: now,
        }),
      { concurrency: "unbounded", discard: true },
    ...
    yield* input.rows.sourceAuthSessions.update(reusableSession.id, {
    ...
    return {
      ...
      authorizationUrl,
    } satisfies Extract<McpSourceConnectResult, { kind: "oauth_required" }>;
  });
...
const upsertSourceOauthClient = (input: {
  ...
  oauthClient: SourceOauthClientInput;
  storeSecretMaterial: StoreSecretMaterial;
  removeSecretMaterial: DeleteSecretMaterial;
  ...
  Effect.gen(function* () {
    ...
    if (
      ...
      yield* input.removeSecretMaterial(previousClientSecretRef).pipe(
        Effect.either,
        Effect.ignore,
      );
    }
...
const upsertProviderAuthGrant = (input: {
  ...
  storeSecretMaterial: StoreSecretMaterial;
  removeSecretMaterial: DeleteSecretMaterial;
  existingGrant?: ProviderAuthGrant | null;
}): Effect.Effect<ProviderAuthGrant, Error, never> =>
  Effect.gen(function* () {
    ...
    if (
      ...
      yield* input.removeSecretMaterial(existingGrant.refreshToken).pipe(
        Effect.either,
        Effect.ignore,
      );
    }
...
const connectMcpSourceInternal = (input: {
  ...
  Effect.gen(function* () {
    ...
    const reusedPendingSession = yield* reusePendingMcpSourceAuthSession({
      rows: input.rows,
      sourceStore: input.sourceStore,
      source: persistedDraft,
      actorAccountId: input.actorAccountId,
      ...
      interactionId: input.interactionId,
      endpoint: normalizedEndpoint,
      preferFreshAuthorization:
        (input.executionId ?? null) === null
        && (input.interactionId ?? null) === null,
    });
    if (reusedPendingSession) {
      return reusedPendingSession;
    }
  ...
  Effect.gen(function* () {
    ...
    if (input.sourceInput.oauthClient) {
      yield* upsertSourceOauthClient({
        ...
        oauthClient: input.sourceInput.oauthClient,
        storeSecretMaterial: input.storeSecretMaterial,
        removeSecretMaterial: input.removeSecretMaterial,
      });
    }
      ...
      provideLocalWorkspace(
        Effect.gen(function* () {
          ...
          if (removed && secretRef) {
            yield* input.removeSecretMaterial(secretRef).pipe(
              Effect.either,
              Effect.ignore,
            );
          }
...
const createRuntimeSourceOAuthSessionService = (
  ...
  completeSourceCredentialSetup: ({ workspaceId, sourceId, actorAccountId, state, code, error, errorDescription }) =>
    provideLocalWorkspace(Effect.gen(function* () {
      ...
      const failSourceCredentialSetup = (reason: string) =>
        Effect.gen(function* () {
          const failedAt = Date.now();
          yield* input.rows.sourceAuthSessions.update(
            session.id,
      ...
      const completion = Effect.gen(function* () {
        ...
        if (trimOrNull(error) !== null) {
          const reason = trimOrNull(errorDescription) ?? trimOrNull(error) ?? "OAuth authorization failed";
          yield* failSourceCredentialSetup(reason);
          return yield* runtimeEffectError("sources/source-auth-service", reason);
        }
        ...
        Effect.catchAll((completionError) =>
          failSourceCredentialSetup(
            completionError instanceof Error ? completionError.message : String(completionError),
          ).pipe(
            Effect.zipRight(Effect.fail(completionError)),
          )),
      );
...
export class SourceAuthService extends Context.Tag(
  "#runtime/SourceAuthService",
  ...
  Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      return createRuntimeSourceAuthService({
        ...
        storeSecretMaterial: secretMaterialStore.store,
        removeSecretMaterial: secretMaterialStore.remove,
        getLocalServerBaseUrl: input.getLocalServerBaseUrl,
