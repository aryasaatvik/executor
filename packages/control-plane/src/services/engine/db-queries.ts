// DB query functions — copied from @executor/engine/src/db/source-state.ts and indexer.ts
...
import { catalog_tool, source } from "./db-schema";
...
export const loadSourceStatus = (sourceId: SourceId) =>
  Effect.gen(function* () {
...
export const loadSourceLifecycle = (sourceId: SourceId) =>
  Effect.gen(function* () {
    const db = yield* SqliteDrizzle;
    const rows = yield* db
...
export const syncSourceLifecycle = (input: {
  ...
  Effect.gen(function* () {
    ...
    yield* db
      ...
      .where(
        eq(
          catalog_tool.sourceId,
          input.sourceId as typeof catalog_tool.$inferInsert.sourceId,
        ),
      );
  });
  ...
  Effect.gen(function* () {
    ...
    const rows = yield* db
      ...
      .where(
        eq(
          catalog_tool.sourceId,
          sourceId as typeof catalog_tool.$inferInsert.sourceId,
        ),
      )
      .limit(1);
