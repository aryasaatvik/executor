...
const toolsFromProjection = (
  ...
  try {
    ...
    for (const [capId, descriptor] of Object.entries(projection.toolDescriptors)) {
      ...
      const interaction = descriptor.interaction.mayRequireApproval ? "confirm" : "auto"
 ...
 * RuntimeSourceStoreService which handles bindings, auth, etc.
 */
...
export const loadSourceStatusFromDb = (
  sourceId: SourceId,
) =>
  Effect.gen(function* () {
    const db = yield* SqliteDrizzle
    const rows = yield* db
      ...
      .limit(1)
    if (rows.length === 0) return null
    return rows[0] as DbSourceStatus
  })

export const upsertSourceStatusToDb = (input: {
  sourceId: SourceId
  workspaceId: WorkspaceId
  name: string
  kind: SourceKind
...
export const removeSourceFromDb = (sourceId: SourceId) =>
  Effect.gen(function* () {
    const db = yield* SqliteDrizzle
