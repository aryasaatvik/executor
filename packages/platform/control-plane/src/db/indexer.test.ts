...
describe("loadSourceStatusFromDb", () => {
  it.effect("returns null when the source does not exist", () =>
    Effect.gen(function* () {
      ...
      const result = yield* loadSourceStatusFromDb(
...
describe("upsertSourceStatusToDb", () => {
...
describe("loadToolForInvocation", () => {
  ...
  it.effect("reconstructs catalog with empty snapshot data when no revision found", () =>
    Effect.gen(function* () {
      ...
      expect(result!.descriptor.interaction).toBe("confirm");
