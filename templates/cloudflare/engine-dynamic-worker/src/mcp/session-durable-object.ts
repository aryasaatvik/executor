...
// ---------------------------------------------------------------------------
// Cloudflare (self-host) MCP Session Durable Object — this host's binding of the
// shared `McpSessionDOBase` adapter. Same base the managed cloud app uses; the
// ONLY differences are the injected dependencies:
...
export class McpSessionDO extends McpSessionDOBase<CfSessionDbHandle> {
  ...
  protected override buildMcpServer(
    ...
    const loader = this.cfEnv.LOADER;
    return Effect.gen(function* () {
      const { engine } = yield* makeExecutionStack(
        sessionMeta.userId,
        sessionMeta.organizationId,
        sessionMeta.organizationName,
      ).pipe(Effect.provide(makeCloudflareExecutionStackLayer(config, dbHandle, loader)));
