...
// ---------------------------------------------------------------------------
// The in-process MCP surface for the local single-user host.
//
// A single-engine Streamable HTTP + stdio MCP server over the ONE boot executor
// (passed in as `engine`), with a browser-approval store for human-in-the-loop
// elicitation. This is local's OWN surface — NOT the shared multi-user
// `McpServingRoutes` envelope. Routed by the Bun shell (serve.ts) at /mcp and
// /api/mcp-sessions/*.
// ---------------------------------------------------------------------------
...
// Local serves these error bodies in-process; they are INNER responses (no CORS).
