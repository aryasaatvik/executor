...
A catalog is the shareable integration identity (table: `catalog`).
...
A catalog is meant to converge across workspaces when the underlying integration definition is the same.

### Revision / materialization

A catalog revision is the compiled result of an import (table: `catalog_revision`).
...
- compile into the same catalog/revision/operation layers
