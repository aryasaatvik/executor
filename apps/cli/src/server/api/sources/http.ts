...
const credentialErrorStatus = (
  error: EngineBadRequestError | EngineNotFoundError | EngineStorageError,
) =>
  error instanceof EngineNotFoundError
    ? 404
    : error instanceof EngineBadRequestError
      ? 400
      : 500;
  ...
  (handlers) =>
    handlers
      ...
      .handle("credentialComplete", ({ path, urlParams }) =>
        resolveRequestedLocalWorkspace("sources.credentialComplete", path.workspaceId).pipe(
          Effect.flatMap(() =>
            completeSourceCredentialSetup({
              workspaceId: path.workspaceId,
              sourceId: path.sourceId,
          ...
          Effect.catchAll((error) => {
            const message = error instanceof Error ? error.message : "Failed completing OAuth";
            return Effect.logError(
              `Failed completing source OAuth for ${path.sourceId}: ${message}`,
            ).pipe(
              Effect.zipRight(
                Effect.succeed(
                  htmlResponse(
